...
var ese_data_room = JSON.parse(sessionStorage.getItem('data_room'));
...
function fonc_list(){
  ...
  //$.getJSON(ese_data_room,
        //function(json){
          var myNode = document.getElementById('miaLista');
        	var hoock=document.getElementById('mioHook');
        	if(myNode){
        		...
        		for(var inHref=0; inHref<ese_data_room.Gruppi_Stanze.length ;inHref++ ){
        				var elmnt=document.createElement('li');
        				var aHref=document.createElement('a');
        				aHref.href='#';
        				aHref.innerText=ese_data_room.Gruppi_Stanze[inHref];

        				aHref.addEventListener('click', function () {
        					var selText = this.textContent;

        					//aHref.parents('.btn-group').find('.dropdown-toggle').html(selText+' <span class='caret'></span>');
        					$(this).parents('.btn-group').find('.dropdown-toggle').innerHtml=selText
        					if (selText !== 'None') 
                    { $('#roomName').val(selText)
                    }else{
                      $('#roomName').val('')
                    };
        					if(hoock.children.length==0){
        						var tipus=[];

        						for(var inHref=0; inHref<ese_data_room.Gruppi_Stanze.length ;inHref++ ){
        							if(ese_data_room.Gruppi_Stanze[inHref]==selText){

                        var trig=0;
        								for (var inGr=0;inGr<ese_data_room.gruppi[selText].length;inGr++){


                          tipus.push(ese_data_room.gruppi[selText][inGr].Tipo);
        										var cusUrl= './icons/'+ese_data_room.gruppi[selText][inGr].Icona
        										var mDiv=document.createElement('div');
        										mDiv.style.setProperty( 'background-color' , '#369' );
        										var mEl=document.createElement('button');

        										mEl.class='btn fully object';
        										mEl.id='gtl';
        										mEl.style.setProperty('display','block');

        										mEl.style.setProperty( 'background-image' , ' url('+'./icons/'+ese_data_room.gruppi[selText][inGr].Icona+')' );
        										mEl.style.setProperty( 'background-repeat' , 'no-repeat' );

        										mEl.style.setProperty( 'background-position','center' );
        										mEl.style.height= '80px' ;
        										mEl.style.width= '100%' ;
        										mEl.style.setProperty( 'padding-left' , ' 44px' );
        										mEl.style.setProperty( 'padding-top' , '70px' );
        										var mLa=document.createElement('LABEL');
        										mLa.innerText=ese_data_room.gruppi[selText][inGr].Nome_Device;

        										mEl.addEventListener('click',function() {

        												cursor('move');
        												$('#boxinfo').html('Add Object');
        												fonc_button('object_mode', this.id);
        										});
        										mDiv.appendChild(mEl);
        										mDiv.appendChild(mLa);
        										hoock.appendChild(mDiv)
        										//console.log('-39-  '  ,' trig   ',trig,  '  !tipus[0]  ',tipus[0],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);


        									//console.log('-33-  '  ,' trig   ',trig,' inGr ',inGr);
                          /*
        									if(!tipus[0]){

        										tipus.push(json.gruppi[selText][inGr].Tipo);
        										var cusUrl= './icons/'+json.gruppi[selText][inGr].Icona
        										var mDiv=document.createElement('div');
        										mDiv.style.setProperty( 'background-color' , '#369' );
        										var mEl=document.createElement('button');
        										...
        										mEl.style.setProperty( 'background-image' , ' url('+'./icons/'+json.gruppi[selText][inGr].Icona+')' );
        										mEl.style.setProperty( 'background-repeat' , 'no-repeat' );
        										...
        										mEl.style.setProperty( 'padding-top' , '70px' );
        										var mLa=document.createElement('LABEL');
        										mLa.innerText=json.gruppi[selText][inGr].Nome_Device;
        										...
        										for(var inTipo=0;inTipo<tipus.length;inTipo++){


        											if (json.gruppi[selText][inGr].Tipo==tipus[inTipo]){
        												if(inTipo==0){

        													trig=1;
        													console.log('-62-  '  ,' trig   ',trig,  '  inTipo  ',inTipo,'  --  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);


        												}
        												else{
        													trig=0;
        													console.log('-68-  '  ,' trig   ',trig,  '  inTipo  ',inTipo,'  --  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);

        												}


        												//console.log('-50-  '  ,' trig   ',trig,  '  trig=1;','  tipus[inTipo]  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);
        											}else{
        													if(trig==1 && json.gruppi[selText][inGr].Tipo!=tipus[inTipo]){

        														console.log('-77-  '  ,' trig   ',trig,  '  /////   else  json.gruppi[selText][inGr].Tipo  !!!!=   tipus[inTipo]  --   con trig=0  ','  tipus[inTipo]  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);
        														tipus.push(json.gruppi[selText][inGr].Tipo);
        														trig=0;
        														var cusUrl= './icons/'+json.gruppi[selText][inGr].Icona
        														var mDiv=document.createElement('div');
        													...
        													if(inTipo!=0){
        														console.log('-82-  '  ,' trig   ',trig,  '  inTipo  ',inTipo,'  --  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);
        														...
        														//console.log('-33-  '  ,' trig   ',trig,  '  /////   else  json.gruppi[selText][inGr].Tipo  !!!!=   tipus[inTipo]  --  //// con trig=1 ---- ','  tipus[inTipo]  ',tipus[inTipo],'  json.gruppi[selText][inGr].name  ',json.gruppi[selText][inGr].Nome_Device);
                          ...
                          */
        						...
        						console.log('120  - tipu   ',tipus);


        				}
        				else{
        						var fc = hoock.firstChild;

        						while( fc ) {
        							hoock.removeChild( fc );
        							fc = hoock.firstChild;
        							}
        				...
        				elmnt.appendChild(aHref);
        				var mL=document.getElementById('miaLista');
        				mL.appendChild(elmnt);
        		}
        	}
        //}
...
function fonc_button(modesetting ,option) {
  ...
  $('.sub').hide();
    ...
    modeOption = option;
    ///
    console.log("modesetting   ",modesetting,"  option   ",option);
...
function carpentryCalc(classObj, typeObj, sizeObj, thickObj, dividerObj = 10) {
  ...
  if (classObj == 'energy') {
    ...
    if (typeObj == 'gtl') {
      ...
      construc.push({
        'image':'./icons/shelly4.svg'
      });
      construc.push({'text': "Shelly4", 'x': '0', 'y':'5', 'fill': "#333333", 'stroke': "none", 'fontSize': '1.3em',"strokeWidth": "0.4px"});
      construc.params.width = 32;
      construc.params.height = 32;
      construc.family = 'stick';
    }
    if (typeObj == 'switch') {
      construc.push({
        'image':'./icons/cancello.svg'
      });
