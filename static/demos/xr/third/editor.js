...
var sclick={
  ssclick:function (){
    ...
    var myArray = sessionStorage.getItem('passaggio');
    var val=JSON.parse(myArray).a

    //const urlParams = new URLSearchParams(window.location.search);

    //var pJ= urlParams.get('greetings');

    //var vMii  =JSON.parse(pJ);
...
*/
  for(var azziz=0;azziz<val.length;azziz++){

    for(var stend=0;stend<val[azziz].length;stend++){
      var sstend = val[azziz][stend]


     var swall = new editor.wall({x:sstend[0][0],y:sstend[0][1]}, {x:sstend[1][0],y:sstend[1][1]}, "normal", 8);
    WALLS.push(swall);
  ...
  roomMaker:  function(Rooms) {
    ...
    for (var rr = 0; rr < ROOM.length; rr++) {
        ...
        qSVG.create('boxSurface', 'path', {
              d: pathCreate,
              fill: '#9e8e73ff', 'fill-opacity': 1, stroke: '#9e8e73ff', 'stroke-width':'12','fill-rule': 'evenodd', class: 'room'});
